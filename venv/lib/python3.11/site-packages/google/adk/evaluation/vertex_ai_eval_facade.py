...
class _VertexAiEvalFacade(Evaluator):
  ...
  def _get_score(self, eval_result) -> Optional[float]:
    if eval_result and eval_result.summary_metrics and self._metric_name:
      metric_key = f"{str(self._metric_name)}/mean"
      return eval_result.summary_metrics.get(metric_key)
